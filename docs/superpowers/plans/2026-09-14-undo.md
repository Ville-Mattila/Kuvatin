...
Run: `cargo test -p kuvatin-video -- --test-threads=1 undo_`
Expected: `test result: ok. 16 passed`. Then run the whole engine suite once to make sure nothing else moved: `cargo test -p kuvatin-video -- --test-threads=1` (the live-media tests need `target/test-fixtures`; they are advisory, as in CI).
...
Expected: all pass. The video line runs the sixteen `undo_` tests and the project-file tests next to them.
