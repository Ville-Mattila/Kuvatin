...
```slint
    ...
    // A drag in progress holds it back (Gesture.held).
...
Run: `cargo test -p kuvatin image_mode::history::`
Expected: `test result: ok. 10 passed`. Dead-code warnings for the new items are expected until Task 10.
...
- Modify: `crates/kuvatin/ui/app.slint` (Images Undo and Redo buttons)
...
- [ ] **Step 8: Put Undo and Redo in the Images Files panel.** The Files panel is 248 px wide inside its padding at the default window size, too narrow for "Add files…", "Clear", "Undo" and "Redo" in one row, so Undo and Redo get a row of their own under it. Keep the row out of an `if`: a hovered control that is destroyed leaves its tooltip on screen. In `crates/kuvatin/ui/app.slint`, replace:
...
**Review fixes (a second commit).** Task 10's review drove the real handlers in a harness and found two ways an Images undo could leave a crop where it did not belong, both older than undo but easy to reach with it:

- With Ctrl+Z held, an undo that restored a file started its 120 ms preview decode, and the next undo removed the file again; the preview still set `edit` to it, so Crop then View wrote a crop for a file not in the list and lost the redo. The preview's success closure now returns unless its file is still the selected one, and `on_apply_crop` applies nothing to a file that is not in the list (`files.binary_search(&path)`).
- After undoing a crop the editor seeds a full frame, so looking in it (Crop then View, untouched) recorded a full-frame crop and lost the redo. An untouched full frame on a file with no crop now applies nothing: `if !listed || (full && !crops.contains_key(&path)) { ui.set_cropping(false); return; }`.
- An undo or redo that selects another file than the one selected closes the crop editor (`if selected != outcome.select { ui.set_cropping(false); }`).
- Clear and remove call `record_step` after `sync_rows` and after dropping their guards, so no UI setter runs with `files` or `crops` locked; Clear moves `old` into the step.
...
**Files:**
- Modify: `.github/workflows/release.yml` (the self-contained filter, and the live-media step)
...
- [ ] **Step 1b: Gate the two undo tests that need real media.** The self-contained step runs before the live-media fixtures are built, so `undo_writes_a_left_trim_of_real_media_back` and `undo_restores_a_trimmed_clip_of_real_media` skip themselves there while reporting ok. In the same file, replace:

```yaml
      # Two regressions that only real media can catch, and both shipped once:
      # an export that came back instantly as a zero-byte file (a stale
      # end-of-stream from the preview was read as the render's result), and a
      # gapped overlay timeline that rendered something no player would open.
      # They used to sit inside the advisory step, where a failure was a line
      # in a log nobody reads. One retry before it counts, because asset-URI
      # resolution on the hosted runner is occasionally flaky — a second
      # failure in a row is the code, not the runner.
      - name: Test (export regressions, live media — gates the release)
        shell: pwsh
        run: |
          ...
          foreach ($attempt in 1..2) {
            ...
            if ($attempt -eq 2) { throw "the export regressions failed twice: $($names -join ', ')" }
            Write-Host "::warning::live-media export regressions failed once; retrying"
          }
```
...
```yaml
      # Regressions that only real media can catch. Two shipped once: an export
      # that came back instantly as a zero-byte file (a stale end-of-stream from
      # the preview was read as the render's result), and a gapped overlay
      # timeline that rendered something no player would open. They used to sit
      # inside the advisory step, where a failure was a line in a log nobody
      # reads. The other two guard undo: a left trim of real media written back,
      # which pins the order of in-point and duration writes, and a trimmed clip
      # restored with its in-point. One retry before it counts, because
      # asset-URI resolution on the hosted runner is occasionally flaky — a
      # second failure in a row is the code, not the runner.
...
| Click in the Duration field and press Ctrl+Shift+Z | Nothing is redone and the field keeps focus; Ctrl+Y in the field redoes the typing |
...
| Crop one, apply, Ctrl+Z, then click Crop and View without moving the rectangle, then Ctrl+Y | Ctrl+Z takes the crop mark away; looking in the editor records nothing; Ctrl+Y brings the crop and its mark back |
| Open Crop on an uncropped file and click View without moving the rectangle | The file is not marked cropped, and Undo does not offer a crop |
| Open Images mode at the default window size | Undo and Redo show in full, on their own row under Add files… and Clear |
| Add three files, remove one, hold Ctrl+Z until the add is undone, click Crop then View, then Ctrl+Y | All three files come back, none marked cropped |
| Crop and apply one file, select another and click Crop, then Ctrl+Z | The first file is selected with its crop gone, and the crop editor is closed |
...
Do not push: the work is on branch `undo`. It is merged to master and pushed after a final review of the whole branch, with the user's go-ahead (superpowers:finishing-a-development-branch). After that push, watch the master run: `gh run list --branch master --limit 1` and `gh run watch <id> --exit-status`. Expected: success, with the self-contained step listing the `undo_` tests and the live-media step running the two that need real media.

**Final review fixes (on the branch, before the merge).** The review of the whole branch found:

- The Images Files panel is 248 px wide inside its padding at the default window size, too narrow for four buttons in one row: Undo was cut off and Redo did not show. Undo and Redo get a row of their own (Task 10, Step 8).
- The timeline chips looked enabled while the video engine was down, though the handler refuses then; both gain `&& !root.video-engine-down` in `enabled`.
- `undo-keys` refuses while a text field has focus (`|| TextInputInterface.text-input-focused` in its first guard), so Ctrl+Shift+Z, which Slint on Windows does not treat as a text redo, no longer redoes a step from a text field and takes the focus away.
- The two undo tests that need real media skipped themselves in the self-contained step while reporting ok; they gate in the live-media step (Step 1b).
- The changelog says that a source deleted from disk does not come back and that opening a project clears the timeline's history, names the zoom chips as the controls that gained hints, and lists the two crop fixes from Task 10's review.

- [ ] **Step 6: Close the backlog item, after the merge and a green master run.** Mark `ui-undo` done on the backlog artifact page (the done list in its `<script id="data">` JSON, with a newer `ts`) and update the `kuvatin-backlog-81` memory. Neither is in the repository.
