...
mod tests {
    ...
    fn step(kind: StepKind, subject: &str, before: &Capture, after: &Capture) -> TimelineStep {
        TimelineStep::new(kind, Some(subject), "intro.mp4", before, after, HashMap::new())
    }
    ...
    fn an_edit_that_changed_nothing_makes_an_empty_step() {
        ...
        assert!(!TimelineStep::new(StepKind::AddTrack, None, "", &same, &one_more_track, HashMap::new()).is_empty());
    }
    ...
    fn a_step_keeps_thumbnails_only_for_clips_that_come_and_go() {
        ...
        let thumbs: HashMap<String, Image> = [("a", Image::default()), ("b", Image::default())]
            ...
            .collect();
        let s = TimelineStep::new(StepKind::Delete, Some("b"), "intro.mp4", &before, &after, thumbs);
        assert!(s.thumbs.contains_key("b"), "b disappears, so its picture is kept");
        assert!(!s.thumbs.contains_key("a"), "a only moved");
    }

    #[test]
    fn moves_of_the_same_clip_merge_and_nothing_else_does() {
        let (c0, c1) = (cap(&[("a", rec(0, 0.0, 2.0))], 2), cap(&[("a", rec(0, 1.0, 2.0))], 2));
        let mv = step(StepKind::Move, "a", &c0, &c1);
        assert!(mv.merges_with(&step(StepKind::Move, "a", &c1, &c0)));
        assert!(!mv.merges_with(&step(StepKind::Trim, "a", &c1, &c0)), "another kind");
        assert!(!mv.merges_with(&step(StepKind::Move, "b", &c1, &c0)), "another clip");
        let del = step(StepKind::Delete, "a", &c0, &c1);
        assert!(!del.merges_with(&step(StepKind::Delete, "a", &c0, &c1)), "deletes never merge");
    }
    ...
    fn renaming_a_clip_rewrites_every_mention_of_it() {
        ...
        let after = cap(&[], 2);
        let thumbs: HashMap<String, Image> = [("old".to_string(), Image::default())].into_iter().collect();
        let mut s = TimelineStep::new(StepKind::Delete, Some("old"), "intro.mp4", &before, &after, thumbs);
    ...
    fn rows_follow_what_was_applied() {
        ...
        let applied = vec![
            Applied { id: "b".into(), now_id: "b".into(), record: None },
            Applied { id: "c".into(), now_id: "c2".into(), record: Some(rec(1, 4.0, 1.0)) },
            Applied { id: "a".into(), now_id: "a".into(), record: Some(rec(0, 3.0, 2.0)) },
        ];
        ...
        assert_eq!(out[1].id.as_str(), "c2", "a restored clip gets the ID it has now");
    ...
    fn the_steps_own_clip_is_selected_first() {
        ...
        assert_eq!(selection_after(&rows, Some("gone"), Some("a")), 0, "else the old selection");
        assert_eq!(selection_after(&rows, None, Some("gone")), -1, "else nothing");
    }

    #[test]
    fn each_kind_describes_itself() {
        let c = cap(&[], 2);
        let d = |kind| TimelineStep::new(kind, Some("a"), "intro.mp4", &c, &c, HashMap::new()).describe();
...
Expected: compile errors for the missing `Capture`, `ClipChange`, `diff`, `StepKind`, `TimelineStep`, `Direction`, `Write`, `plan`, `target_tracks`, `Applied`, `rows_after`, `selection_after`.
...
pub(super) fn plan(step: &TimelineStep, dir: Direction) -> Vec<Write> {
    ...
    for change in &step.changes {
        ...
        match (from, to) {
            ...
            (None, Some(record)) => restores.push(Write::Restore(change.id.clone(), record.clone())),
...
plus the thumbnails of clips that come or go and the track-row count. Pure
...
```rust
    ...
    fn assert_same_record(a: &crate::document::ClipRecord, b: &crate::document::ClipRecord) {
        assert_eq!((a.uri.as_str(), a.track), (b.uri.as_str(), b.track), "source and track");
        for (x, y, what) in [
        ...
        assert_eq!((a.layout.posx, a.layout.posy), (b.layout.posx, b.layout.posy));
    ...
    fn undo_reports_a_write_the_engine_refused() {
        ...
        onto_b.start = 2.0;
        assert_eq!(project.set_clip_records(&[(a.clone(), onto_b)]), vec![a.clone()]);
        assert!(project.clip_track(&a).is_some(), "a is still on the timeline");
        let _ = std::fs::remove_dir_all(&dir);
    }
```
...
moves is parked alone on a new layer first, and every clip is read back, so
a refusal is reported rather than shown as done.
...
**If `undo_restores_a_deleted_clip_under_its_old_id` fails only at `assert_eq!(restored, a, "same ID")`**, GES refuses to reuse the name. The design's fallback is already built in: `restore_clip` returns the new ID and Task 7 rewrites the old one through `History::steps_mut` and `TimelineStep::rename_clip`. Replace that one assertion with:

```rust
        assert!(project.clip_track(&restored).is_some(), "restored, under GES's name");
```

and add a line to the spec's Risks section recording that GES does not reuse names. Every other assertion must still pass.
...
restore_clip re-adds a removed clip from its record and asks GES for the old
name first, so the IDs the interface and the undo history hold stay valid.
source_available is the check undo makes before bringing a clip back, so a
file deleted in the meantime is named rather than failing at preroll.
...
Run: `cargo test -p kuvatin-video -- --test-threads=1 undo_`
Expected: `test result: ok. 11 passed`. Then run the whole engine suite once to make sure nothing else moved: `cargo test -p kuvatin-video -- --test-threads=1` (the live-media tests need `target/test-fixtures`; they are advisory, as in CI).
...
```rust
    ...
    /// the clip's name and keeps its thumbnail for the undo.
    #[test]
    fn the_recorder_names_the_clip_and_keeps_its_picture() {
        ...
        assert!(s.thumbs.contains_key("a"));
    }
...
```rust
/// What recording a step needs: the history, the rows (for a clip's name and
/// thumbnail) and the track rows. Cheap to clone into each handler.
#[derive(Clone)]
...
impl Recorder {
    ...
    /// the name and thumbnail come from the rows.
    pub(super) fn record(
        &self,
    ...
    pub(super) fn record_captures(
        ...
        let thumbs = rows
            .iter()
            .map(|r| (r.id.to_string(), r.thumb.clone()))
            .collect();
        let step = TimelineStep::new(kind, subject, &name, &before, &after, thumbs);
...
```rust
    ...
    if let Some(row) = tl_clips.row_data(i as usize) {
        if let Some(p) = project_slot.borrow_mut().as_mut() {
            let before = rec.before(Some(&*p));
            p.remove_clip(&kuvatin_video::ClipId(row.id.to_string()));
            // Recorded before the row goes, so the step keeps its thumbnail.
...
fn apply_step(
    ...
    let (writes, tracks, subject, thumbs) = {
    ...
    let missing: Vec<String> = writes
        .iter()
        .filter_map(|w| match w {
            Write::Restore(_, record) if !p.source_available(&record.uri) => {
    ...
    for w in &writes {
        match w {
            ...
            Write::Set(id, record) => {
                sets.push((kuvatin_video::ClipId(id.clone()), record.clone()));
            }
            Write::Restore(..) => {}
        }
    }
    let refused = p.set_clip_records(&sets);
    for (id, record) in sets {
    ...
    if failed.is_none() {
        for w in &writes {
            let Write::Restore(id, record) = w else {
                continue;
            };
    ...
    // On a failure, the rows follow the engine rather than the plan.
    let to_rows = if failed.is_some() {
        applied_from_engine(&rows, p.clip_records())
    } else {
        applied.clone()
    };
    let new_rows = rows_after(&rows, &to_rows, &thumbs);
    ...
    // A restore may have handed back a new ID: every step must use it now.
...
```bash
git add crates/kuvatin/src/gui/video/undo.rs crates/kuvatin/src/gui/video/mod.rs
git commit -F - <<'MSG'
The timeline's undo and redo run

Applying a step checks that every clip it brings back still has its file,
then removes clips, writes the rest in one batch and restores, in that
order, prunes tracks the step did not have, updates only the affected rows and
selects the clip the step was about. If the engine refuses a write partway, the rows are rebuilt from what
the engine holds and the step stays. A clip restored under a new ID has that
ID rewritten in every step.
...
- Modify: `crates/kuvatin/ui/app.slint` (imports, `modal-open`, `undo-keys`, the `kbd` handler, the timeline toolbar, the tooltip layer)
...
```slint
    ...
    function undo-keys(event: KeyEvent) -> EventResult {
        if (!event.modifiers.control || root.modal-open()) { return EventResult.reject; }
...
Expected: all pass. The video line runs the eleven `undo_` tests and the project-file tests next to them.
...
trim, clips trading places in one batch, a refused write reported, a deleted
clip restored under its own ID, tracks pruned back, a missing source named)
