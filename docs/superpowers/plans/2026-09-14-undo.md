...
| `crates/kuvatin-video/src/project.rs` | Modify | `set_clip_record`, `restore_clip`, `source_available`, `prune_tracks`, engine tests |
...
Run: `cargo test -p kuvatin history::`
Expected: `test result: ok. 14 passed`. Until later tasks use it, `cargo build` warns that `History` and its methods are unused; that is expected and goes away in Task 6.
...
(removals, then restores, then writes), and work out the rows and selection
afterwards.
...
### Task 3: Engine — write a clip's record back exactly

**Files:**
- Modify: `crates/kuvatin-video/src/project.rs` (new method after `set_clip_duration`, around line 985; tests at the end of `mod tests`)
...
```rust
    ...
    fn undo_writes_a_clamped_slide_back_exactly() {
        ...
        assert!(project.set_clip_record(&b, &before));
    ...
    fn undo_writes_a_clamped_trim_back_exactly() {
        ...
        assert!(project.set_clip_record(&a, &before));
    ...
    /// every clip back on its old track.
    #[test]
    fn undo_puts_clips_back_after_a_track_reorder() {
        ...
        assert!(project.set_clip_record(&a, &ra));
        assert!(project.set_clip_record(&b, &rb));
...
Expected: `error[E0599]: no method named set_clip_record found for struct Project`.

- [ ] **Step 3: Implement.** In `crates/kuvatin-video/src/project.rs`, directly after the closing brace of `pub fn set_clip_duration`, add:

```rust
    /// Write a clip's start, in-point, duration, track and transform exactly as
    /// `record` gives them. Nothing is clamped: this puts back a state the
    ...
    /// Returns false for an unknown clip or while rendering.
    pub fn set_clip_record(&mut self, id: &ClipId, record: &crate::document::ClipRecord) -> bool {
        ...
        // The order trim_clip uses, so in-point plus duration never passes the
        // source's max-duration on the way, which GES would clamp.
        ...
        if clip.layer().map(|l| l.priority() as usize) != Some(record.track) {
            let target = self.layer(record.track);
            if clip.move_to_layer(&target).is_err() {
                return false;
            }
        }
...
git commit -F - <<'MSG'
The engine can put a clip back exactly

set_clip_record writes a clip's start, in-point, duration, track and
transform as given, without the clamping slide and trim apply. Undo needs
this: a slide that stopped against a neighbour, replayed in reverse, is
clamped again and lands somewhere else.
...
**Files:**
- Modify: `crates/kuvatin-video/src/project.rs` (new methods after `set_clip_record`; tests at the end of `mod tests`)
...
- [ ] **Step 3: Implement.** Directly after `set_clip_record`, add:
...
Run: `cargo test -p kuvatin-video -- --test-threads=1 undo_`
Expected: `test result: ok. 9 passed`. Then run the whole engine suite once to make sure nothing else moved: `cargo test -p kuvatin-video -- --test-threads=1` (the live-media tests need `target/test-fixtures`; they are advisory, as in CI).
...
In the test module add, after `use kuvatin_video::LayoutRecord;`:
...
Expected: the build succeeds; all tests pass (the earlier 87 plus 14 history and 15 undo tests). The only warnings allowed are dead-code warnings for `plan`, `target_tracks`, `Applied`, `rows_after`, `selection_after`, `Direction`, `Write` and the engine methods, which Task 7 uses.
...
fn apply_step(
    ...
    for w in &writes {
        match w {
            ...
            Write::Set(id, record) => {
                if p.set_clip_record(&kuvatin_video::ClipId(id.clone()), record) {
                    applied.push(Applied {
                        id: id.clone(),
                        now_id: id.clone(),
                        record: Some(record.clone()),
...
then removes, restores and writes clips in that order, prunes tracks the step
did not have, updates only the affected rows and selects the clip the step was
about. If the engine refuses a write partway, the rows are rebuilt from what
...
Expected: all pass. The video line runs the nine `undo_` tests and the project-file tests next to them.
...
trim, a deleted clip restored under its own ID, tracks pruned back, a missing
source named) join the video tests that gate a release.
