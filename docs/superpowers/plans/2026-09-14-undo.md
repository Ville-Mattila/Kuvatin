...
**Files:**
- Modify: `crates/kuvatin/ui/widgets.slint` (`SecondaryButton` lines 95–133, `TimelineChip` lines 606–640, new `Tooltip` and `TooltipLayer` at the end)
- Modify: `crates/kuvatin/ui/app.slint` (imports, `modal-open`, `undo-keys`, the `kbd` handler, `pointer-gesture` on five touch areas, the timeline toolbar, the tooltip layer)
...
```slint
    ...
    // A drag in progress holds it back (see pointer-gesture).
    function undo-keys(event: KeyEvent) -> EventResult {
        if (!event.modifiers.control || root.modal-open() || root.pointer-gesture) { return EventResult.reject; }
...
- [ ] **Step 5b: Hold undo back during a drag.** A clip drag keeps its row index until the pointer is released, and the preview's transform drag keeps writing to the selected clip: an undo in between would move the rows or the selection under the gesture. In `crates/kuvatin/ui/app.slint`, make these five replacements, each adding one line (or, for the first, the property):

For the property and its helper, next to `clip-dragging`, replace:
...
```slint
    in-out property <bool> clip-dragging: false;
    // True while the pointer is held on a clip, a trim grip, a track header
    // or the preview's transform box. Undo waits for it: a drag holds a row
    // index that an undo would move under it.
    in-out property <bool> pointer-gesture: false;
    // Whether a pointer gesture is still going on after `ev`.
    pure function gesture-after(ev: PointerEvent) -> bool {
        return ev.kind == PointerEventKind.down
            || (root.pointer-gesture && ev.kind != PointerEventKind.up && ev.kind != PointerEventKind.cancel);
    }
```
...
```slint
                                            pointer-event(ev) => {
                                                root.pointer-gesture = root.gesture-after(ev);
                                                if (ev.kind == PointerEventKind.up) {
                                                    if (blk.dmode == DragMode.slide && (Math.abs(blk.ddx / 1px) > 2 || Math.abs(blk.ddy / 1px) > 2)) {
```
